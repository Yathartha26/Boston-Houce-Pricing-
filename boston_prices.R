...
#Boston Pricing case study using Linear Regression
...
#summary statistics 
summary(prices)
table(prices$Charles.River.dummy.variable)

##########Missing values treatment 
colSums(is.na(prices)) 
summary(prices$MEDV) #MEDV has a lot of missing values
...
############Outlier Treatment##############
plot(prices$MEDV,prices$Charles.River.dummy.variable)
plot(prices$MEDV,prices$ZN)

plot(prices$INDUS,prices$MEDV)
...
list<-list[-4] 
for(i in 1:length(list)) #Plot the boxplots of all variables and shortlist wh ich ones need outlier treatment. 
{ 
  boxplot(prices[,list[i]],main=list[i]) 
} 

#Restore the par parameters to normal
#Figure margin too large, check par('mar'), you will get 5.1 4.1 4.1 2.1 change it to mar=c(1,1,1,1)
par(mar=c(1,1,1,1))
...
# We can replace the outlier values by the median values 
#You can decide to replace by max or mean values based on business objectives 

for(i in 1:length(list)) ##For loop to replace all the outlier values with th e mean value ;
...
#Significant variables are : B LSTAT AGE X.rooms.dwelling nitric.oxides.conce ntration INDUS 
...
##     DIS + ZN + PTRATIO + LSTAT + AGE + X.rooms.dwelling + nitric.oxides.co ncentration 
